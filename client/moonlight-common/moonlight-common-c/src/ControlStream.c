...
#define IDX_CURSOR_STATE 11

#define SS_CURSOR_STATE_V1_FIXED_LENGTH 46
...
static const short packetTypesGen7Enc[] = {
    ...
    0x5504, // Cursor state (Sunshine protocol extension; 0x5502/0x5503 are used by RGB/adaptive triggers)
};
...
static void controlReceiveThreadFunc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&controlReceiveThread)) {
        ...
        if (event.type == ENET_EVENT_TYPE_RECEIVE) {
            ...
            else if (ctlHdr->type == packetTypes[IDX_CURSOR_STATE]) {
                uint32_t payloadLength = packetLength - sizeof(*ctlHdr);
                if (payloadLength >= SS_CURSOR_STATE_V1_FIXED_LENGTH) {
                    uint8_t* cursorPayload = (uint8_t*)ctlHdr + sizeof(*ctlHdr);
                    uint8_t version = cursorPayload[0];
                    uint8_t flags = cursorPayload[1];
                    uint32_t sequence = readLe32(cursorPayload + 2);
                    int32_t x = (int32_t)readLe32(cursorPayload + 6);
                    int32_t y = (int32_t)readLe32(cursorPayload + 10);
                    int32_t clipLeft = (int32_t)readLe32(cursorPayload + 14);
                    int32_t clipTop = (int32_t)readLe32(cursorPayload + 18);
                    int32_t clipRight = (int32_t)readLe32(cursorPayload + 22);
                    int32_t clipBottom = (int32_t)readLe32(cursorPayload + 26);
                    uint16_t width = readLe16(cursorPayload + 30);
                    uint16_t height = readLe16(cursorPayload + 32);
                    uint16_t hotspotX = readLe16(cursorPayload + 34);
                    uint16_t hotspotY = readLe16(cursorPayload + 36);
                    uint32_t cursorHash = readLe32(cursorPayload + 38);
                    uint32_t imageByteLength = readLe32(cursorPayload + 42);
                    const uint8_t* imageData = cursorPayload + SS_CURSOR_STATE_V1_FIXED_LENGTH;

                    if (imageByteLength <= payloadLength - SS_CURSOR_STATE_V1_FIXED_LENGTH) {
                        ListenerCallbacks.cursorState(version, flags, sequence, x, y,
                            clipLeft, clipTop, clipRight, clipBottom, width, height,
                            hotspotX, hotspotY, cursorHash, imageData, imageByteLength);
                    }
                }
            }
