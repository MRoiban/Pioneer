...
static PSDP_OPTION getAttributesList(char*urlSafeAddr) {
    ...
    if (StreamConfig.cursorFeedback) {
        err |= addAttributeString(&optionHead, "x-ss-client.cursorFeedback", "1");
    }

    // Sunshine extensions for low-latency encoding
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        // Sunshine reads this to track the user's configured bitrate independent of negotiated peak
